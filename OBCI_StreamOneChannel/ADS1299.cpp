...
//    pinMode(DAISY_SS,OUTPUT);
//    pinMode(SD_SS,OUTPUT);
//    pinMode(LIS3DH_SS,OUTPUT);
//    digitalWrite(DAISY_SS,HIGH);  // de-select Daisy Module
//    digitalWrite(SD_SS,HIGH);  // de-select SD Card
//    digitalWrite(LIS3DH_SS,HIGH);  // de-select LIS3DH
...
void ADS1299::writeChannelDataAsBinary(int N, uint16_t sampleNumber)
{
	//check the inputs
	if ((N < 1) || (N > 8)) return;
	...
	//if (sampleNumber >= 0) {
		Serial.write(sampleNumber); //4 bytes long
	//}
...
void ADS1299::writeChannelDataAsOpenEEG_P2(long sampleNumber) {
	...
	for (int chan = 0; chan < 6; chan++ )
	{
		if (false) {
			...
			//val32 = channelData[chan] + (long)0x00008000; //shift
			val32 = channelData[chan]; //shift
			val32 = constrain(val32,min_int16,max_int16);
			val_u16 = (unsigned int) (val32 & 0x0000FFFF); //get just the lower 16-bits
		} else {
			//get the higher 16-bits
			val32 = channelData[chan] / (32) ; //turn 24-bits into 16-bits 
			val32 = constrain(val32,min_int16,max_int16);
			val32 += max_int16; //shift upward
			
			//val32 = ((long)(chan-3))*11000L;
			//val32 = val32 + 0x8000;
			
			if (chan == 0) val32 = sampleNumber;  //dummy value
	
			val_u16 = (unsigned int) (val32 & (0x0000FFFF));
			//val_i16 = (int)((val32 >> 8) & (0x0000FFFF));
			//val_u16 = (unsigned int)(val_i16 + 0x8000);
			//val_u16 = (unsigned int)(val_i16);
		}
